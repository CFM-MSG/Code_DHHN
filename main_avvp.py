...
def retrain_no_MGN(args, model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        log_str = 'Train Epoch: {} [{}/{} ({:.0f}%)]\t audio Loss: {:.3f}\t video Loss: {:.3f}\t wsl Loss: {:.3f}\t  adv: {:.3f}'.format(
                epoch, batch_idx * len(audio), len(train_loader.dataset), 100. * batch_idx / len(train_loader), loss1.item(),  loss2.item(), loss3.item(), 0 )
        loss = loss #+ loss8
        loss.backward(retain_graph=True)
        optimizer[0].step()
        if batch_idx % args.log_interval == 0:
...
def retrain_MGN_single(args, model, train_loader, optimizer, criterion, epoch, mgn):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        if mgn=='audio':
            audio_pred, _, _, _ = audio_model(audio, video, video_st)
            audio_pred.clamp_(min=1e-7, max=1 - 1e-7)
            loss_mgn = criterion(audio_pred, Pa) 
            
            loss_dis = F.mse_loss(a_prob, audio_pred.detach())
            optz = optimizer[1]
           
        else:
            video_pred, _, _, _ = video_model(audio, video, video_st)
            video_pred.clamp_(min=1e-7, max=1 - 1e-7)
            loss_mgn = criterion(video_pred, Pv)
            # loss7 = criterion3(audio_pred, video_pred)
            # loss7 = -F.mse_loss(audio_pred, video_pred)
        
            loss_dis = F.mse_loss(v_prob, video_pred.detach())
            optz = optimizer[2]

        log_str = 'Train Epoch: {} [{}/{} ({:.0f}%)]\t audio Loss: {:.3f}\t video Loss: {:.3f}\t wsl Loss: {:.3f}\t dist_a: {:.3f}\t, adv: {:.3f}; \n[SubNet]\t audio_mgn loss: {:.3f}'.format(
                epoch, batch_idx * len(audio), len(train_loader.dataset), 100. * batch_idx / len(train_loader), loss1.item(),  loss2.item(), loss3.item(), loss_dis.item(), 0, 
                loss_mgn.item())
        
        loss = loss + loss_dis
        loss.backward(retain_graph=True)
        loss_mgn.backward(retain_graph=True)
        # for optz in optimizer:
        #     optz.step()
        optimizer[0].step()
        optz.step()
...
def main():
    ...
    parser.add_argument(
        ...
        help="save model name")
    parser.add_argument(
        "--mgn-single", type=str, default='',
        help="mgn subnet")
    parser.add_argument(
        "--no-log", action='store_true', default=False,
        help="logger switcher")
    parser.add_argument(
        "--no-mgn", action='store_true', default=False,
        help="logger switcher")
        ...
        for epoch in range(1, args.epochs + 1):
            if not args.no_mgn:
                if args.mgn_single == "":
                    ...
                    retrain_MGN_single(args, model, train_loader, optimizer, criterion, epoch=epoch, mgn=args.mgn_single)

            else:
                retrain_no_MGN(args, model, train_loader, optimizer, criterion, epoch=epoch)
